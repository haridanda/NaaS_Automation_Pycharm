...
def post_Create_UNI(PCID, naas_config, data_provider, input_dict):
...
def validate_UNI_ASRI(naas_config, data_provider, input_dict):
    print(input_dict)
    Asri_URL = naas_config.get_server("asri-url") + get_asri + input_dict['asriServiceAlias']
    response = requests.get(Asri_URL, json=request_body, headers=headers, verify=False)
    response_json = response.json()
    #input_list = ['asriServiceAlias', 'actId']
    #flag = 0
    ...
    print(status)
    assert type == "uni" , "type Incorrect in ASRI"
    assert  status == "In Service", "status NOT IN Service"
...
def validate_Service_ASRI(naas_config, data_provider, input_dict):
    print(input_dict)
    Asri_URL = naas_config.get_server("asri-url") + get_asri + service_alias
    response = requests.get(Asri_URL, json=request_body, headers=headers, verify=False)
    response_json = response.json()
    #input_list = ['asriServiceAlias', 'actId']
    #flag = 0
    ...
    status = response_json['resources'][0]["status"]
    print(status)
    assert type == "Internet Access" , "type Incorrect in ASRI"
    assert  status == "In Service", "status NOT IN Service"
